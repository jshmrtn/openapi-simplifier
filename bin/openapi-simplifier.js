...
const { simplifySchema } = require('../dist/index')
const fs = require('fs')
...
const outputWriter = output ? (content) => fs.writeFileSync(output, content) : (content) => process.stdout.write(content)

const content = fs.readFileSync(inputFile === '-' ? 0 : inputFile).toString()
const simplfiedContent = simplifySchema(content)
